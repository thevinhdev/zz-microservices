...
{
    public class GetByPageGuestApartmentQuery : FilterPagination, IRequest<IPagedResult<ResidentDT>>
    {
        public long? ResidentId { get; set; }
        public int? ApartmentId { get; set; }

        public class Handler : IRequestHandler<GetByPageGuestApartmentQuery, IPagedResult<ResidentDT>>
        {
            ...
            public async Task<IPagedResult<ResidentDT>> Handle(GetByPageGuestApartmentQuery request, CancellationToken cancellationToken)
            {
                ...
                {
                    //def.meta = new Meta(222, Const.NOPERMISION_VIEW_MESSAGE);
                    //return Ok(def);
                ...
                var entities = await _entityRepository.GetByPageMemberApartmentAsync(filterPagination, request.ApartmentId, cancellationToken);
