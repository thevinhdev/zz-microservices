...
{
    public class GetResidentByUserMapIdQuery : IRequest<UserReceiveNotification>
    {
        public long UserMapId { get; set; }

        public class Handler : IRequestHandler<GetResidentByUserMapIdQuery, UserReceiveNotification>
        {
            ...
            public async Task<UserReceiveNotification> Handle(GetResidentByUserMapIdQuery request, CancellationToken cancellationToken)
            {
                UserReceiveNotification res = (from u in _userRepo.All().Where(u => u.Status != AppEnum.EntityStatus.DELETED && u.UserMapId == request.UserMapId && (u.Type == (int)AppEnum.TypeUser.RESIDENT_GUEST || u.Type == (int)AppEnum.TypeUser.RESIDENT_MAIN))
