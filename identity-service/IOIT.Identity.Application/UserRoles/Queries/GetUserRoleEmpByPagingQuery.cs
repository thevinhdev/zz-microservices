...
{
    public class GetUserRoleEmpByPagingQuery : FilterPagination, IRequest<ResUserRoleLists>
    {
        ...
        public class Handler : IRequestHandler<GetUserRoleEmpByPagingQuery, ResUserRoleLists>
        {
            ...
            public async Task<ResUserRoleLists> Handle(GetUserRoleEmpByPagingQuery request, CancellationToken cancellationToken)
            {
                ...
                var entities = await _userRepo.GetByPageEmpAsync(filterPagination, request.roleMax, request.roleLevel, request.ProjectId, cancellationToken);
                ...
                foreach (var item in entities.Results)
                {
                    ...
                    user.UserCreateId = item.UserCreateId;
                    user.UserEditId = item.UserEditId;
                    ...
                    var listRole = await _userRoleRepo.GetListRoleAsync(item.UserId, cancellationToken);
                    ...
                    //}).ToListAsync();
                    var listFunction = await _funcRoleRepo.GetListFunctionRoleAsync(item.UserId, (int)AppEnum.TypeFunction.FUNCTION_USER, cancellationToken);
